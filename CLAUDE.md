...
| `biweekly` | 兩欄目一張稿：①過去 14 日全站熱門前 10（同 weekly 邏輯）②過去 14 日內**曾被置頂過**的所有文章，按頻道分組。先補當日快照、tag 作者 | 隔週三下午（與週報同日、錯開）| `digest-biweekly.yml` |

cron 以 UTC 計，每日快照 07:00 HKT = 23:00 UTC。**週報／雙週報都排「主＋備」兩個錯開的冷門時段**：
週報**每週三上午** 01:37＋04:37 UTC（09:37＋12:37 HKT）；雙週報**隔週三下午** 05:37＋08:37 UTC
（13:37＋16:37 HKT）——同一「雙週報週三」兩份錯開時段、不相撞。因 GitHub 排程常延遲甚至整點 run 被丟棄。
...
1. **不用官方 `hottest` feed**：官方演算法偏「新」而非互動，會漏掉互動高但稍舊的文。
   改為合併各頻道、用「拍手＋留言」透明計分，數字可逐篇核對。**資格門檻：至少 1 個拍手
